...
unsigned int simTime = 100;
uint16_t nTrackers = 2;
short nRelays = 10;
short nVictims = 2;
...
int main  (int argc, char *argv[])
{
	...
	{
		Ptr<NetDevice> relayInterface = DynamicCast<MeshPointDevice> (*iter)->GetInterface(1);
	...
	move_drone(trackers.Get (0), victims.Get (0)->GetObject<MobilityModel>()->GetPosition(), 10);
	move_drone(trackers.Get (1), victims.Get (1)->GetObject<MobilityModel>()->GetPosition(), 10);
